...
- **正确判据 = "是否在 Worker 运行时"**：`navigator.userAgent === 'Cloudflare-Workers'`。
  ...
  - 构建一律用本地 binding（动态路由构建期不查库）。生产实测证明 `payload run`、`payload migrate` 与 `tsx + getPlatformProxy(remoteBindings)` 都会受 Miniflare 远程代理失败影响：seed 曾部分写入后 exit 0，大迁移持续收到 D1 `internal error`。因此生产 seed 与 migration 均走原生 `wrangler d1 execute --remote --file`，随后做独立 D1 JSON 断言；任一步失败均非零退出。
  ...
  - 不允许把空 `CLOUDFLARE_ENV` 展开为 `--env=`：OpenNext deploy wrapper 可能只打印 remote connection 后 exit 0，却不创建 deployment。app/database deploy 改由 Node 仅在非空时追加 `--env <name>`；OpenNext 生成 bundle 后由 `wrangler deploy` 直接发布；前后读取 `wrangler versions list --json`，若没有新 Worker version 则强制失败。
...
6. **⚠️ D1 迁移不会随 `deploy:app` 自动跑（首次上线前必须解决）**：`deploy:app` 只打包+部署 Worker，建表迁移在 `deploy:database`。若不专门执行，D1 里没有表，应用运行时报错、站点起不来。方案：
   - **推荐**：GitHub→Cloudflare 的部署命令用 `pnpm run deploy`（= `deploy:database` 迁移 + `deploy:app`），并在构建环境注入 `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`。`deploy:database` 从提交的 Payload migration 静态提取 `up()` SQL，使用原生 Wrangler D1 应用并校验 migration row + schema sentinels，绕开 Miniflare remote proxy。
   - **首次替代**：先本地/手动带 token 跑一次 `pnpm run deploy:database`，之后再交给 CI 自动迁移。
