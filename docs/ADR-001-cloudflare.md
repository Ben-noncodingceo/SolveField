...
- **正确判据 = "是否在 Worker 运行时"**：`navigator.userAgent === 'Cloudflare-Workers'`。
  ...
  - `remoteBindings` 仅在 **Payload CLI**（迁移）或显式 `SOLVEFIELD_REMOTE_BINDINGS=1` 的独立脚本（当前仅 `seed:remote`）且有 token 时为 true；构建一律用本地 binding（动态路由构建期不查库）。生产 seed 不使用会 exit-0/no-op 的 `payload run`，而由 `tsx` 直接执行并校验写入结果。
